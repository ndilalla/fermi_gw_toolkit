...
def adaptive_time(**kwargs):
    ...
    if plot:
        ...
        rot=(orgx,orgy)
        hp.mollview(ligo_map/p_max, sub=221, title='Ligo Map',cmap=cool_cmap,rot=rot, coord='C')
        hp.graticule()
        hp.mollview(ligo_selected, sub=222, title='Ligo Map (%.1f c.l.)' % kwargs['cl'],cmap=cool_cmap,norm='lin',min=0,max=1.0,rot=rot, coord='C')
        hp.graticule()
        hp.mollview(ligo_entry, sub=223, title='Entry Time',cmap=jet,norm='lin',min=times_t0.min(),max=times_t0.max(),rot=rot, coord='C')
        hp.graticule()
        hp.mollview(ligo_expo, sub=224, title='Exposure (s)',cmap=cool_cmap,norm='log',min=0.1,max=max(ligo_expo),rot=rot, coord='C')
        hp.graticule()
        ax = plt.gca()
