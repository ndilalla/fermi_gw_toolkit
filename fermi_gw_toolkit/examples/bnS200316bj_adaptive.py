...
OUTULMAP = os.path.join(OUTPUT_FILE_PATH, 'ATI_ul_map.fits')
OUTTSMAP = os.path.join(OUTPUT_FILE_PATH, 'ATI_ts_map.fits')
...
def run():
    ...
    for i in range(0,2):
        ...
        ra, dec, tstart, tstop, deltat = float(ra), float(dec), float(tstart),\
                                         float(tstop), float(deltat)
